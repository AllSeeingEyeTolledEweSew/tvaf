...
def test_insert(atp: lt.add_torrent_params, conn: apsw.Connection) -> None:
...
def test_ignore(atp: lt.add_torrent_params, conn: apsw.Connection) -> None:
    atp.save_path = "expected"
    split = resumedb.split_resume_data(atp)
    resumedb.insert_or_ignore_resume_data(split.info_hashes, split.resume_data, conn)
    if split.info is not None:
